...
import { RestProvider, SettingsProvider, NotifProvider, WooCommerceProvider, ToastProvider, UserProvider } from './../providers/providers';
import { OneSignal } from '@ionic-native/onesignal';
import { Component, NgZone, ViewChild } from '@angular/core';
import { Platform, Events, ModalController, AlertController, Nav, IonicApp, MenuController } from 'ionic-angular';
...
export class MyApp {
  ...
  constructor(private admob: AdMobFree, public platform: Platform, private statusBar: StatusBar, public oneSignal: OneSignal, private zone: NgZone,
    public settings: SettingsProvider, public translate: TranslateService, private events: Events, rest: RestProvider,
    private toast: ToastProvider, private modalCtrl: ModalController, private emailComposer: EmailComposer, private appRate: AppRate,
    private alertCtrl: AlertController, private appVersion: AppVersion, private WC: WooCommerceProvider, public user: UserProvider,
    ionicApp: IonicApp, menuCtrl: MenuController, private notif: NotifProvider, private iab: InAppBrowser, private anlyticsService: AnalyticsService,
    private loader: LoadingProvider, private imageLoaderConfig: ImageLoaderConfig) {
    ...
    platform.ready().then(() => {
      ...
      if (platform.is('android')) {
        ...
        platform.registerBackButtonAction(() => {
          let activePortal = ionicApp._loadingPortal.getActive() || ionicApp._modalPortal.getActive() ||
  ...
  initApp() {
    this.WC.saveAppSettings().subscribe((res) => {
      console.log(res);
      this.settings.setSettings(res, 'appSettings').then(() => {
        ...
        const colors = new Map([
          ...
          ['primary_color_dark', this.settings.all.appSettings.primary_color_dark],
          ['accent_color', this.settings.all.appSettings.accent_color],
          ['toolbar_text_color', this.settings.all.appSettings.primary_color_text],
          ['toolbar-badge-color', this.settings.all.appSettings.toolbarbadgecolor || this.settings.all.appSettings.accent_color],
        ...
        if (this.platform.is('cordova')) {
          ...
          this.statusBar.backgroundColorByHexString(this.settings.all.appSettings.primary_color_dark);
          this.appVersion.getVersionNumber().then((v) => {
            this.version = v;
            if (this.settings.all.appSettings.google_analytics_tracker_id && this.settings.all.appSettings.google_analytics_tracker_id !== '') {
              this.anlyticsService.init(this.settings.all.appSettings.google_analytics_tracker_id).then(() => {
  ...
  exit() {
    this.translate.get(['CONFIRM', 'EXIT_MSG', "EXIT", 'CANCEL']).subscribe((x) => {
      this.alertCtrl.create({
        title: x.CONFIRM,
        message: x.EXIT_MSG,
        buttons: [{
