...
export class WooCommerceProvider {
  ...
  woo: any = {};
  RestURL: string;
  constructor(public storage: Storage, public http: HttpClient, public temp: SettingsProvider) {
  ...
  //   });
  // }
  getAllCategories(parent?) {//: Promise<any> {
    let pt = parent ? "?parent=" + parent : '';
    return this.http.get(this.RestURL + "products/all-categories" + pt);
  }
  getSubCategories(c: any) {//: Promise<any> {
    return this.WooCommerce.getAsync("products/categories?parent=" + c).then((x) => {
      console.log(JSON.parse(x.body));
      return JSON.parse(x.body);
    });
  }
  ...
  //   });
  // }
  getProductById(include?, id?) {//Multiple ids suppoerted(include=1,2,3,4,5) or single id supported(id=1)
  ...
  getCustomProducts(page, per_page, sort, category, brand, search, min_price?, max_price?, on_sale?, featured?, filterData?) {
    ...
    return this.http.post(this.RestURL + "custom-products" + p + pp + s + c + b + sh + min_p + max_p + os + f, filterData);
  }
  getProductThumb(id) {
  ...
  getProductAttributes(id?: number) {
    let i = id ? "?id=" + id : "";
    return this.WooCommerce.getAsync("products/product-attributes/" + i).then((x) => {
      // console.log(JSON.parse(x.body));
      return JSON.parse(x.body);
    });
  }
  ...
  getCustomAttributes(product?) {
    ...
    return this.http.get(this.RestURL + "products/custom-attributes?hide_empty=true" + p);
  }


  getProductVariation(data) {  //Custom api made by phoeniixx
    return this.http.post(this.RestURL + "products/get-variation", data);
  }
  getOrders(id: number, page: number, per_page: number) {
    let p = page ? '&page=' + page : '';
    let pp = per_page ? '&per_page=' + per_page : '&per_page=10';
    return this.WooCommerce.getAsync('orders?customer=' + id + p + pp).then((data) => {
  ...
  getDownloads(id: number) {
    return this.WooCommerce.getAsync("customers/" + id + "/downloads").then((x) => {
      // console.log(JSON.parse(x.body));
      return JSON.parse(x.body);
    });
  }
  ...
  checkPincode(pincode, product_id?) {
    let p: any = {};
    p.pincode = pincode;
    if (product_id) { p.product_id = product_id }
  ...
  getReviewSettings(product_id, user_id) {
    ...
    return this.http.get(this.RestURL + "product/review-settings" + p_id + u_id).subscribe((x) => {
