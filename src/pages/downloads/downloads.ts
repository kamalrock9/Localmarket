...
export class DownloadsPage {
  ...
  constructor(
    ...
    this.WC.getDownloads(user.id).then(
      (data) => {
        console.log(data);
        this.zone.run(() => {
          this.downloads = data;
  ...
  downloadNow(url, fileName) {
    ...
    this.loader.show();
    this.translate
      .get(["OK", "DOWNLOAD_SUCCESS", "DOWNLOAD_FAILED"])
      .subscribe((x) => {
        fileTransfer
          .download(
            encodeURI(url),
            this.storageDirectory + fileName + "." + this.get_url_extension(url)
          )
          .then(
            (entry) => {
              this.loader.dismiss();
            ...
            (error) => {
              ...
              this.alertCtrl
                .create({
                  ...
                  subTitle:
                    fileName + " was not downloaded. Error code: " + error.code,
                  buttons: [x.OK],
                })
                .present();
            }
