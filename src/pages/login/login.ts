...
import { UserProvider, LoadingProvider, ToastProvider, SettingsProvider } from './../../providers/providers';
...
export class LoginPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public viewCtrl: ViewController, private settings: SettingsProvider,
    private formBuilder: FormBuilder, private user: UserProvider, private loader: LoadingProvider, platform: Platform,
    private toast: ToastProvider, private translate: TranslateService, private googlePlus: GooglePlus,
    private fb: Facebook) {
  ...
  dismiss(skip) {
    if (skip) {
      if (this.signupby === 'social') {
        this.translate.get(['LOGIN_SUCCESS'], { value: this.user.first_name || this.user.username }).subscribe(x => {
  ...
  login() {
    ...
    this.user.login(this.loginForm.value).subscribe((res: any) => {
      ...
      this.loader.dismiss();
      if (res.code == 1) {
        this.translate.get(['LOGIN_SUCCESS'], { value: res.details.first_name || res.details.username }).subscribe(x => {
          this.toast.show(x.LOGIN_SUCCESS);
        });
  ...
  if (!this.signupForm.valid) {
    ...
    this.loader.show();
    this.user.register(this.signupForm.value).subscribe((res: any) => {
      console.log(res);
      this.loader.dismiss();
      if (res.status == '1') {
  ...
  reset() {
    ...
    this.loader.show();
    this.user.reset(this.resetForm.value).subscribe((res: any) => {
  ...
  googleLogin() {
    ...
    this.googlePlus.login({})
      .then(res => {
        res.mode = 'google';
        console.log(res);
        this.user.socialLogin(res).subscribe((res: any) => {
          console.log(res);
          this.loader.dismiss();
          if (res.code == 1) {
            this.user.loggedIn(res.details);
  ...
  facebookLogin() {
    this.fb.login(['public_profile', 'email'])
      .then((res: FacebookLoginResponse) => {
        ...
        this.fb.api('me?fields=id,name,email,first_name,last_name', []).then((res) => {
          console.log(res);
          res.mode = "facebook";
          if (res.email && res.email !== '') {
            ...
            this.translate.get(['FB_EMAIL_REQUIRED'], { value: res.details.first_name || res.details.username }).subscribe(x => {
  ...
  applyReferralCode() {
    ...
    this.loader.show();
    this.user.applyReferralCode(this.currentUserId, this.referralForm.value.refer_code).subscribe((x: any) => {
      this.loader.dismiss();
      if (x.status == 1) {
